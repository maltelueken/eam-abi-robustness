...
from data import save_hdf5, load_hdf5
...
def fit_mcmc(cfg: DictConfig):
    ...
    logger.info("Running MCMC with parameters %s", (p1_, p2_))

    if cfg["with_params"]:
        model = model_fun(sim_data[idx, :, :], p1_, p2_)
    else:
