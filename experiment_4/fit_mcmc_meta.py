...
def fit_mcmc(cfg: DictConfig):
    ...
    for filename in os.listdir(os.path.join(cfg["test_data_path"], "test_data")):
        ...
        model_fun_vec = np.vectorize(model_fun, signature="(m,n)->()", excluded=[1, 2, 3, 4, 5])
        ...
        model_funs_with_data = model_fun_vec(sim_data, p1, p2)
        ...
        # Save to hdf5 file
        save_hdf5(os.path.join("mcmc_samples", f"fit_mcmc_{os.path.splitext(filename)[0]}.hdf5"), {"samples": np.array(list(trace_data.values()))})
