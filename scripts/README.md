...
| `predict_npe.py` | Sample the trained NPE's posterior for every case. |
...
| `check_robustness.py` | Maximum mean discrepancy between the NPE and MCMC posteriors. |
...
On the cluster, use `slurm/submit.sh` rather than calling these directly.
