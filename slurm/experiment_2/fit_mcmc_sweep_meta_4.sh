...
do
    ...
    do
        for i in {0..99}
        do
            srun --exclusive --ntasks=1 python experiment_2/fit_mcmc_slurm.py experiment=experiment_2 model=rdm_simple_meta +slurm_p1=$p1 +slurm_p2=$p2 +slurm_idx=$i &
        done
        wait
        python experiment_2/collect_mcmc.py experiment=experiment_2 model=rdm_simple_meta +slurm_p1=$p1 +slurm_p2=$p2
