"""Utilities to create a racing diffusion model in PyMC and perform MCMC sampling."""
...
def standard_normal_logcdf(t):
    """Log cumulative density of the standard normal distribution."""
    return pt.switch(
        pt.lt(t, -1.0),
...
def inv_gauss_logsf(t, mu, lam):
    """Log survival density of the Inverse Gaussian distribution.
    
    See: https://journal.r-project.org/archive/2016-1/giner-smyth.pdf
    ...
    return pt.switch(
        pt.isposinf(t),
        -np.inf,
        pt.switch(pt.gt(t, 0.0), a + pt.log1p(-pt.exp(b - a)), 0.0),
    )


class RdmSimpleRV(RandomVariable):
    """PyMC model for the racing diffusion model."""
    ...
    @classmethod
    def rng_fn(
        cls, rng, drift_winner, drift_loser, s_winner, s_loser, threshold, ndt, size
    ) -> np.ndarray:
...
class RdmSimple(Continuous):
    """PyMC distribution for the racing diffusion model."""
    rv_op = rdm
    ...
    def dist(
        ...
        **kwargs,
        ...
        return super().dist(
            [drift_winner, drift_loser, s_winner, s_loser, threshold, ndt], **kwargs
        )

    def support_point(
        rv, size, drift_winner, drift_loser, s_winner, s_loser, threshold, ndt
    ):
        drift_winner, _, _, _, _, _ = pt.broadcast_arrays(
            drift_winner, drift_loser, s_winner, s_loser, threshold, ndt
        )
    ...
    def logp(
        value, drift_winner, drift_loser, s_winner, s_loser, threshold, ndt, min_p=1e-10
    ):
        value = value - ndt
        ...
        logp = pt.switch(
            ...
            inv_gauss_logsf(value, mu_loser, lam_loser),
            pt.log(min_p),
        )

        logp = pt.maximum(
            pt.switch(pt.isinf(logp) | pt.isnan(logp), pt.log(min_p), logp),
            pt.log(min_p),
        )
...
def rdm_model_simple(sim_data, drift_slope_loc, threshold_scale):
    """Create a PyMC instance of the racing diffusion model."""
...
def rdm_model_meta(
    sim_data,
    drift_slope_loc_lower,
    drift_slope_loc_upper,
    threshold_scale_lower,
    threshold_scale_upper,
):
    """Create a PyMC instance of the racing diffusion model with varying priors."""
    rt_true = sim_data[sim_data[:, 1] == 1, 0]
    rt_false = sim_data[sim_data[:, 1] == 0, 0]

    with pm.Model() as model:
        drift_slope_loc = pm.Uniform(
            "drift_slope_loc", lower=drift_slope_loc_lower, upper=drift_slope_loc_upper
        )
        threshold_scale = pm.Uniform(
            "threshold_scale", lower=threshold_scale_lower, upper=threshold_scale_upper
        )
...
def run_mcmc(
    ...
    **kwargs,
    ...
    """Perform MCMC inference with a PyMC logdensity function."""
    if rng_key is None:
